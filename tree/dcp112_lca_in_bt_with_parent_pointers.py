...
###############################################################################
"""
Because parent pointer is available, it's easy to find the path from a 
given node to the root of tree.  

We traverse this path for the first node, putting each node in a set.
...
O(n) time.
O(log n) time if we used a self-balancing binary tree.
"""
def lca(x, y):
    if x == y:
        return x
    ...
    # traverse path to root from other given node, checking against set
    while y:
        if y in path:
            return y
        y = y.parent
